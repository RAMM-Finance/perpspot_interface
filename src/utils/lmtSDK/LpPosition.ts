...
import { encodeSqrtRatioX96, maxLiquidityForAmounts, Pool, SqrtPriceMath, TickMath, tickToPrice } from '@uniswap/v3-sdk'
...
export class LmtLpPosition {
  ...
  public mintAmountsWithSlippage(slippageTolerance: Percent): Readonly<{ amount0: JSBI; amount1: JSBI }> {
    ...
    const positionThatWillBeCreated = LmtLpPosition.fromAmounts({
      pool: this.pool,
    ...
    const { amount0 } = new LmtLpPosition({
      pool: poolUpper,
    ...
    const { amount1 } = new LmtLpPosition({
      pool: poolLower,
      liquidity: positionThatWillBeCreated.liquidity,
      tickLower: this.tickLower,
      tickUpper: this.tickUpper,
  ...
  public customMintAmountsWithSlippage(
    mintAmount0: JSBI,
    mintAmount1: JSBI,
  ...
  public static fromAmount0({
    ...
    return LmtLpPosition.fromAmounts({ pool, tickLower, tickUpper, amount0, amount1: MaxUint256, useFullPrecision })
  }
