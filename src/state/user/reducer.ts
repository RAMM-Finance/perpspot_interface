  ...
  reducers: {
    ...
    updatePinnedPools(state, action) {
      if (action.payload.add) {
        state.pinnedPools[action.payload.chainId].push(action.payload.poolKey)
      } else {
        const id2 = `${action.payload.poolKey.token0.toLowerCase()}-${action.payload.poolKey.token1.toLowerCase()}-${
          action.payload.poolKey.fee
        }`
        const index = state.pinnedPools[action.payload.chainId].findIndex((i) => {
          const { token0, token1, fee } = i
          const id = `${token0.toLowerCase()}-${token1.toLowerCase()}-${fee}`
          return id === id2
        })
        if (index >= 0) {
          state.pinnedPools[action.payload.chainId].splice(index, 1)
        }
      }
    },
    setPinnedPools(state, action) {
      state.pinnedPools[action.payload.chainId] = action.payload.pinnedPools
    },
    setInputCurrency(state, action) {
      if (state.currentPools[action.payload.chainId]) {
        state.currentPools[action.payload.chainId].inputInToken0 = action.payload.inputInToken0
      }
    },
    setCurrentPool(state, action) {
      state.currentPools[action.payload.chainId] = {
