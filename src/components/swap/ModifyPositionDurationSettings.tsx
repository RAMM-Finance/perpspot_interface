...
export default function ModifyPositionDurationSettings({ estValue }: { estValue?: string | number }) {
  const [open, setOpen] = useState(false)
  // const [initial, setInitial] = useState(estValue)
  ...
  const { onEstimatedDurationChange } = useMarginTradingActionHandlers()
    ...
    <AutoColumn gap="md" style={{ padding: '.5rem' }}>
      <RowBetween align="start">
        ...
        <MouseoverTooltip text="Reset to default position duration">
          ...
          <ResetWrapper onClick={() => onEstimatedDurationChange('')}>
